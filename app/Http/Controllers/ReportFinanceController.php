...
class ReportFinanceController extends Controller
{
    ...
    public function index()
    {
        ...
        // public function getBankBalanceGraph(Request $request) {
        //     try {
        //         $limit = $request->has('limit') ? $request->limit : 10;
        
        //         $bankBalances = BankBalance::paginate($limit);
        //         $currencyCount = Currency::where('status', 1)->count();
        //         $financeAccCount = FinanceAccount::where('status', 1)->count();
                
        //         if ($bankBalances->isEmpty()) {
        //             return response()->json([]);
        //         }
        
        //         // Prepare data for chart (only amounts)
        //         $chartData = [];
        //         foreach ($bankBalances as $bankBalance) {
        //             $chartData[] = $bankBalance->amount;
        //         }
        
        //         // Return data including chart data
        //         return response()->json([
        //             'bank_balance' => $bankBalances,
        //             'currency_count' => $currencyCount,
        //             'finance_account_count' => $financeAccCount,
        //             'chart_data' => $chartData,
        //             'total_pages' => $bankBalances->lastPage()
        //         ]);
    ...
    // Rooznacha
            ...
            public function getRooznamcha(Request $request){
                try {
                   ...
                   ->with(['financeAccount','customer','tr_currency','eq_currency','bank_account','referencedTransaction'])->orderBy('id','desc')
                    ...
                    return response()->json(['transactions' =>$transaction,'currencies' => $currency,'customers' => $customers,'financeAccounts' => $financeAccount ]);
                }
                catch (Exception $e) {
            ...
            public function getTransactionCountandMonth(){
                try {
                    $transactions = Transaction::select(
                        DB::raw('DATE_FORMAT(date, "%Y-%m-%d") as month'),
